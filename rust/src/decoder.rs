...
impl Decoder {
    pub fn new() -> Self {
        ...
        Self {
            ...
            luma_scratch: FilterScratch::new(luma_fft_n),
            iq_scratch: FilterScratch::new(iq_fft_n),
    ...
    pub fn decode_frame(
        ...
        for vis in 0..VISIBLE_LINES {
            ...
            // cos(φ + π/2·k): [cos_phi, -sin_phi, -cos_phi, sin_phi]
            let cos_lut = [cos_phi, -sin_phi, -cos_phi, sin_phi];
            // sin(φ + π/2·k): [sin_phi, cos_phi, -sin_phi, -cos_phi]
        ...
        for vis in 0..VISIBLE_LINES {
            let base = vis * spl;
            for v in &mut self.chroma_full[base..base + ACTIVE_START] {
                *v = 0.0;
            }
        }
        ...
        for vis in 0..VISIBLE_LINES {
            ...
            // Phase at sample 0: φ_i = line_phase + bp + I_PHASE_RAD
            ...
            // cos(φ + π/2·n) for n%4: [cos, -sin, -cos, sin]
        ...
        if !need_resize {
            ...
            for vis in 0..VISIBLE_LINES {
                ...
                for n in 0..ACTIVE_SAMPLES {
                    ...
                    output[out_base + n * 3] = (r * 255.0).round() as u8;
                    output[out_base + n * 3 + 1] = (g * 255.0).round() as u8;
                    output[out_base + n * 3 + 2] = (b * 255.0).round() as u8;
                }
            }
        } else {
